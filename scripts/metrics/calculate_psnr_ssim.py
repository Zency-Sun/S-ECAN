...
def main(args):
    ...
    for i, img_path in enumerate(img_list_gt):
        ...
        print(f'{i+1:3d}: {basename:25}. \tPSNR: {psnr:.6f} dB, \tSSIM: {ssim:.6f}')  # 2023.6.13为方便结果输出，注释
    ...
    # 将数据写入 2023.6.13
    path = r"/media/zency/SSD-Data/Codes/BasicSR/data.txt"
    with open(path,'a') as file:
        file.write(str(sum(psnr_all) / len(psnr_all)))
        file.write("\t")
        file.write(str(sum(ssim_all) / len(ssim_all)))
        file.write("\n")
